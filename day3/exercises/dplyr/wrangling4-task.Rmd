...
output2 <- tidy_users %>% 
  filter( Users > 50 & Weekday == "Saturday" ) %>%
#  filter( Users > 50, Weekday == "Saturday" ) %>%  # comma == &
  arrange( desc(Users) )
...
output3 <- tidy_users %>%
  group_by( Weekday ) %>%
  summarise( Visitors = mean(Users) ) %>%  # declare new column Visitors
  arrange( Visitors )
...
output4 <- output3 %>%
  mutate( mean_users = round(Visitors) )
